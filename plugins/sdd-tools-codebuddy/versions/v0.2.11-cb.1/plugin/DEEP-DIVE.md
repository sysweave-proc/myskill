...
The **sdd-tools** plugin implements a complete Spec-Driven Development (SDD) pipeline for CodeBuddy. It transforms the development process from ad-hoc prompting into a structured workflow: **idea → spec → tasks → execution**. The plugin is fully standalone (no external plugin dependencies) and provides 4 skills, 4 agents, and a lifecycle hook, enabling developers to go from a product idea to working code through an automated, verification-driven pipeline.
...
flowchart TD
    ...
    subgraph Phase3["Phase 3: Decomposition"]
        CT["/create-tasks"]
        SPEC -->|"reads"| CT
        CT -->|"creates"| TASKS["~/.codebuddy/tasks/{list}/*.json"]
    end
    ...
    subgraph Phase4["Phase 4: Execution"]
        ...
        AGENTS -->|"writes"| CTX[".codebuddy/sessions/__live_session__/"]
        ET -->|"updates"| TASKS
    end
...
| create-tasks | Spec file | Task JSON files | CodeBuddy native tasks |
...
Transforms a specification into a dependency-ordered set of CodeBuddy native Tasks, each with categorized acceptance criteria, testing requirements, and metadata for tracking.
...
flowchart TD
    ...
    S9 --> S10["Step 10: Update CODEBUDDY.md"]
...
```mermaid
flowchart TD
    INIT["Initialize Session"] --> DIR[".codebuddy/sessions/__live_session__/"]
    ...
    POINTER["~/.codebuddy/tasks/{list}/execution_pointer.md"] --> DIR

    COMPLETE["Session Complete"] --> ARCHIVE[".codebuddy/sessions/{execution_id}/"]
...
| **Purpose** | Auto-approves file operations within `.codebuddy/sessions/` directories |
| **Timeout** | 5 seconds |
...
```mermaid
flowchart LR
    ET["/execute-tasks"] --> |"creates/updates"| JSON["~/.codebuddy/tasks/*.json"]
...
Settings are configured in `.codebuddy/agent-alchemy.local.md` (not committed):
