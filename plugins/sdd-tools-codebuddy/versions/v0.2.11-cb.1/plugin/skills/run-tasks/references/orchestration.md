...
```
Input:  Filtered task set, .codebuddy/agent-alchemy.local.md (optional)
Output: Execution plan with wave assignments
```

### 2a: Read Settings

Read `.codebuddy/agent-alchemy.local.md` if it exists. Parse YAML frontmatter between `---` delimiters for settings under the `run-tasks` namespace. If the file is missing or any setting is absent, use the default silently (missing settings file is not an error).
...
1. Attempt to read `.codebuddy/agent-alchemy.local.md`. If the file does not exist, use all defaults and continue silently.
...
3. Parse the extracted YAML. If parsing fails (malformed YAML), log a warning message and use all defaults:
   ```
   Warning: Malformed YAML in .codebuddy/agent-alchemy.local.md — using default settings.
   ```
...
**Example `.codebuddy/agent-alchemy.local.md`**:
```yaml
---
...
Check whether `.codebuddy/sessions/__live_session__/` exists and contains files. Detection is based on directory content presence — there is no `.lock` file in the new engine.
...
2. Move all contents of `.codebuddy/sessions/__live_session__/` to `.codebuddy/sessions/interrupted-{timestamp}/`.
3. Reset any `in_progress` tasks from the interrupted session to `pending` via `TaskUpdate`.
4. Log the archival:
   ```
   Archived interrupted session to .codebuddy/sessions/interrupted-{timestamp}/.
...
1. Check `~/.codebuddy/teams/` for directories matching the pattern `wave-*`
...
3. If `~/.codebuddy/teams/` does not exist or contains no `wave-*` directories, skip this step silently
...
Create `.codebuddy/sessions/__live_session__/` (if it does not already exist) and populate it with the initial session files:
...
```
Session initialized: .codebuddy/sessions/__live_session__/
...
Session ID: {session_id}
Session Dir: {absolute path to .codebuddy/sessions/__live_session__/}
Context Manager Model: {context_manager_model}
Executor Model: {executor_model}
...
4. **Clean up failed team** (aggressive — skip cooperative shutdown in crash scenarios):
   a. Read `~/.codebuddy/teams/{wave-team-name}/config.json` to enumerate all team members (the wave-lead constructs the team name as `wave-{N}-{session_id}`).
...
Check whether `~/.codebuddy/teams/{wave-team-name}/` directory still exists (the wave-lead constructs the team name as `wave-{N}-{session_id}`):
...
**0. Check for abort signal**: Check whether `.codebuddy/sessions/__live_session__/.abort` exists.
...
Read `task_log.md` and `execution_context.md` to build a comprehensive session report. Write `session_summary.md` to `.codebuddy/sessions/__live_session__/`:
...
1. Create the archive directory: `.codebuddy/sessions/{session-id}/`
2. Move all files from `.codebuddy/sessions/__live_session__/` to `.codebuddy/sessions/{session-id}/`.
3. Leave `.codebuddy/sessions/__live_session__/` as an empty directory (ready for the next session).

**Archive race condition**: If `.codebuddy/sessions/{session-id}/` already exists (extremely unlikely — would require two sessions with the same timestamp), append a counter: `{session-id}-2`, `{session-id}-3`, etc.
...
```
Input:  execution_context.md (from archived session), CODEBUDDY.md
Output: CODEBUDDY.md edits (if warranted)
```

### 7a: Review Execution Context

Read the archived `execution_context.md` from `.codebuddy/sessions/{session-id}/execution_context.md` and evaluate whether any learnings represent **project-wide changes** that should be reflected in `CODEBUDDY.md`.

**Changes that warrant CODEBUDDY.md updates**:
...
**Changes that do NOT warrant CODEBUDDY.md updates**:
...
### 7b: Update CODEBUDDY.md

If meaningful changes were identified:

1. Read the current `CODEBUDDY.md`.
...
```
No CODEBUDDY.md updates needed. All learnings were task-specific.
```
...
```
Execution complete. Session archived to .codebuddy/sessions/{session-id}/.
```
