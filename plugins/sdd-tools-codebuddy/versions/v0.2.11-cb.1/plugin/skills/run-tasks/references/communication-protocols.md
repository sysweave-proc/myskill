...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/references/messaging-protocol.md
```
...
| `Session Dir` | Required | Absolute path to `.codebuddy/sessions/__live_session__/` |
...
Session Dir: /Users/dev/my-project/.codebuddy/sessions/__live_session__/
...
For agent-level anti-patterns that lead to malformed messages (AP-03: Missing activeForm, AP-06: TaskList-Only Consumption), see the claude-code-tasks anti-patterns reference (`${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/references/anti-patterns.md`).
...
3. If team directory still exists (wave-lead failed to delete, or crashed before TeamDelete):
   a. Read `~/.codebuddy/teams/{wave-team-name}/config.json` to enumerate team members
...
1. Orchestrator scans `~/.codebuddy/teams/` for directories matching `wave-*` patterns from previous sessions
...
1. Reads `~/.codebuddy/teams/wave-{N}-{session_id}/config.json` to enumerate all team members
