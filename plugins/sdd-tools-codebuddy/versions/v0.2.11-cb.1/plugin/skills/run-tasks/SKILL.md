---
name: run-tasks
description: Execute pending tasks in dependency order with wave-based concurrent execution via Agent Teams 中文触发（用户这样说时使用）：用 Agent Teams 跑任务、波次并行执行、先 dry-run 看执行计划、按任务组或阶段执行。
version: 0.2.11
...
### Tasks Reference
```
Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/SKILL.md
```

### Teams Reference
```
Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/SKILL.md
```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/references/orchestration-patterns.md
```

### Orchestration Reference
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/run-tasks/references/orchestration.md
```
...
Read settings from `.codebuddy/agent-alchemy.local.md` (use defaults if the file is missing). Build the execution plan:
...
2. **Check for existing `__live_session__/` content**: If found, offer the user a choice via `AskUserQuestion`: resume (reset `in_progress` tasks to `pending`) or fresh start (archive to `.codebuddy/sessions/interrupted-{timestamp}/`).
3. **Create session artifacts** in `.codebuddy/sessions/__live_session__/`:
...
- Archive: Move `__live_session__/` contents to `.codebuddy/sessions/{session-id}/`.

See `references/orchestration.md` Step 6 for the summary format and archival procedure.

### Step 7: Finalize

Review `execution_context.md` for project-wide changes and update `CODEBUDDY.md` if warranted:
...
See `references/orchestration.md` Step 7 for the CODEBUDDY.md update criteria.
...
- **Graceful abort**: Users can stop execution between waves by creating `.codebuddy/sessions/__live_session__/.abort` from another terminal. The current wave completes, remaining tasks are marked failed, and the session is archived. Optionally include an abort reason as file content (e.g., `echo "requirements changed" > .codebuddy/sessions/__live_session__/.abort`).
...
This skill uses CodeBuddy hooks for automated quality gates during execution:
...
Hook definitions are in `${CODEBUDDY_PLUGIN_ROOT}/hooks/hooks.json`. For hook event documentation, see `claude-code-teams/references/hooks-integration.md`.
...
```
touch .codebuddy/sessions/__live_session__/.abort
# Or with a reason:
echo "requirements changed" > .codebuddy/sessions/__live_session__/.abort
```
...
- `references/orchestration.md` — Detailed 7-step orchestration procedures, wave execution, retry escalation, session management, and CODEBUDDY.md update criteria
...
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/SKILL.md` — Task tool parameters and conventions (loaded at init)
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/SKILL.md` — Team lifecycle, messaging, and orchestration patterns (loaded at init)
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/references/orchestration-patterns.md` — 6 orchestration patterns (loaded at init, for context)
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/references/messaging-protocol.md` — SendMessage types, delivery mechanics, shutdown handshake (loaded by agents)
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-teams/references/hooks-integration.md` — TeammateIdle/TaskCompleted hook events
