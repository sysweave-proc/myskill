---
name: create-tasks
description: Generate CodeBuddy native Tasks from an existing spec. Use when user says "create tasks", "generate tasks from spec", "spec to tasks", "task generation", or wants to decompose a spec into implementation tasks. 中文触发（用户这样说时使用）：把规格拆成任务、从 SPEC 生成任务、任务分解、拆解实现步骤。
version: 0.2.11
...
You are an expert at transforming specifications into well-structured, actionable implementation tasks. You analyze specs, decompose features into atomic tasks, infer dependencies, and create CodeBuddy native Tasks with proper metadata and acceptance criteria.
...
**CRITICAL**: This skill generates tasks, NOT an implementation plan. When invoked during CodeBuddy's plan mode:
...
Before starting the workflow, load the CodeBuddy Tasks reference for tool parameters, conventions, and patterns:
...
1. Check `.codebuddy/agent-alchemy.local.md` for a default spec directory or output path, and try resolving the spec path against it
...
Check for optional settings at `.codebuddy/agent-alchemy.local.md`:
...
```
/create-tasks specs/SPEC-Auth.md --phase 1
```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/references/anti-patterns.md
```
...
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/SKILL.md` — Task tool parameters, conventions, and patterns (loaded at init)
- `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-claude-tools/skills/claude-code-tasks/references/anti-patterns.md` — Common task anti-patterns (loaded on error/validation)
