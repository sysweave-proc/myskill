---
name: analyze-spec
description: Analyze an existing spec for inconsistencies, missing information, ambiguities, and structure issues. Use when user says "analyze spec", "review spec", "spec quality check", "validate requirements", "audit spec", or "check spec quality". 中文触发（用户这样说时使用）：审一下这份规格、规格有没有遗漏或矛盾、需求质量检查、校验需求、规格体检。
version: 0.2.11
...
Check for settings at `.codebuddy/agent-alchemy.local.md` to get:
