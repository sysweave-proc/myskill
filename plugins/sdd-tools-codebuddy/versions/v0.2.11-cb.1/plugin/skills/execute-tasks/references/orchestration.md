...
This reference provides the detailed 10-step orchestration loop for executing CodeBuddy Tasks in dependency order. The execute-tasks skill uses this procedure to manage the full execution session.
...
Agents write `result-task-{id}.md` in `.codebuddy/sessions/__live_session__/`:
...
- **Empty task list**: Report "No tasks found. Use `/create-tasks` to generate tasks from a spec, or create tasks manually with TaskCreate." and stop.
...
2. `max_parallel` setting in `.codebuddy/agent-alchemy.local.md`
3. Default: 5
...
Read `.codebuddy/agent-alchemy.local.md` if it exists, for any execution preferences.
...
Before creating new files, check if `.codebuddy/sessions/__live_session__/` contains leftover files from a previous session (e.g., due to interruption):

1. Check if `.codebuddy/sessions/__live_session__/` exists and contains any files
2. If files are found:
   - Create `.codebuddy/sessions/interrupted-{YYYYMMDD}-{HHMMSS}/` using the current timestamp
   - Move all contents from `__live_session__/` to the interrupted archive folder
   - Log: `Archived stale session to .codebuddy/sessions/interrupted-{YYYYMMDD}-{HHMMSS}/`
...
1. Check if `.codebuddy/sessions/__live_session__/.lock` exists
...
After the concurrency check passes, create `.codebuddy/sessions/__live_session__/.lock` with:
...
Create `.codebuddy/sessions/__live_session__/` (and `.codebuddy/sessions/` parent if needed) with:
...
5. **`progress.md`** - Initialize with status template:
   ...
   ```
6. **`execution_pointer.md`** at `$HOME/.codebuddy/tasks/{TASK_LIST_ID}/execution_pointer.md` — Create immediately with the fully resolved absolute path to the live session directory (e.g., `/Users/sequenzia/dev/repos/my-project/.codebuddy/sessions/__live_session__/`). Construct this by prepending the current working directory to `.codebuddy/sessions/__live_session__/`. This ensures the pointer exists even if the session is interrupted before completing. `{TASK_LIST_ID}` is the identifier of the current task list (visible from `TaskList`); if the runtime does not expose such an identifier, use the task-list name — or a constant like `default` — consistently for the whole session.

## Step 6: Initialize Execution Context

Read `.codebuddy/sessions/__live_session__/execution_context.md` (created in Step 5.5).

If a prior execution session's context exists, look in `.codebuddy/sessions/` for the most recent timestamped subfolder and merge relevant learnings (Project Patterns, Key Decisions, Known Issues, File Map) into the new execution context.
...
Read `.codebuddy/sessions/__live_session__/execution_context.md` and hold it as the baseline for this wave. All agents in this wave will read from this same snapshot. This prevents concurrent agents from seeing partial context writes from sibling tasks.
...
Task:
  ...
  prompt: |
    ...
    Context Write Path: .codebuddy/sessions/__live_session__/context-task-{id}.md
    Result Write Path: .codebuddy/sessions/__live_session__/result-task-{id}.md
    ...
    2. Read .codebuddy/sessions/__live_session__/execution_context.md for prior learnings
    ...
    7. Write learnings to .codebuddy/sessions/__live_session__/context-task-{id}.md
    8. Write result to .codebuddy/sessions/__live_session__/result-task-{id}.md
...
**Poll invocation** (via Bash tool with `timeout: 2760000`):
   ...
   ```bash
   bash ${CODEBUDDY_PLUGIN_ROOT}/skills/execute-tasks/scripts/poll-for-results.sh \
     .codebuddy/sessions/__live_session__ {task_id_1} {task_id_2} {task_id_3}
   ```
...
2. **Read result files**: For each task in the wave, read `.codebuddy/sessions/__live_session__/result-task-{id}.md`. Parse:
...
**Context append fallback**: If a result file is missing but `TaskOutput` contains a `LEARNINGS:` section, manually write those learnings to `.codebuddy/sessions/__live_session__/context-task-{id}.md`.
...
1. Read `.codebuddy/sessions/__live_session__/execution_context.md`
2. Read all `context-task-{id}.md` files from `.codebuddy/sessions/__live_session__/` in task ID order
...
1. Archive completed task files: for each PASS task in this wave, copy the task's JSON from `~/.codebuddy/tasks/{TASK_LIST_ID}/` to `.codebuddy/sessions/__live_session__/tasks/`
...
1. Save `session_summary.md` to `.codebuddy/sessions/__live_session__/` with the full summary content
2. **Archive the session**: Create `.codebuddy/sessions/{task_execution_id}/` and move all contents from `__live_session__/` to the archival folder. The `.lock` file is moved to the archive along with all other session files, releasing the concurrency guard.
...
## Step 9: Update CODEBUDDY.md

Review `.codebuddy/sessions/{task_execution_id}/execution_context.md` for project-wide changes that should be reflected in CODEBUDDY.md.

Update CODEBUDDY.md if the session introduced:
...
Do NOT update CODEBUDDY.md for:
...
- Tasks are executed using CodeBuddy's native task system (TaskGet/TaskUpdate/TaskList)
