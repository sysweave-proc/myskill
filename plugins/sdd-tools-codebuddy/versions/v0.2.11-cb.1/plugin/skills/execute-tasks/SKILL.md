---
name: execute-tasks
description: Execute pending CodeBuddy Tasks in dependency order with wave-based concurrent execution and adaptive verification. Supports task group filtering and configurable parallelism. Use when user says "execute tasks", "run tasks", "start execution", "work on tasks", or wants to execute generated tasks autonomously. 中文触发（用户这样说时使用）：执行任务、把任务跑起来、按依赖顺序自动实现、继续跑没做完的任务。
version: 0.2.11
...
- What conventions the project follows (CODEBUDDY.md, existing patterns)
...
Resolve `max_parallel` setting using precedence: `--max-parallel` CLI arg > `.codebuddy/agent-alchemy.local.md` setting > default 5. Build a dependency graph from pending tasks. Assign tasks to waves using topological levels: Wave 1 = no dependencies, Wave 2 = depends on Wave 1 tasks, etc. Sort within waves by priority (critical > high > medium > low > unprioritized), break ties by "unblocks most others." Cap each wave at `max_parallel` tasks.

### Step 4: Check Settings
Read `.codebuddy/agent-alchemy.local.md` if it exists for execution preferences, including `max_parallel` setting. CLI `--max-parallel` argument takes precedence over the settings file value.

### Step 5: Initialize Execution Directory
Generate a `task_execution_id` using three-tier resolution: (1) if `--task-group` provided → `{task_group}-{YYYYMMDD}-{HHMMSS}`, (2) else if all open tasks share the same `metadata.task_group` → `{task_group}-{YYYYMMDD}-{HHMMSS}`, (3) else → `exec-session-{YYYYMMDD}-{HHMMSS}`. Clean any stale `__live_session__/` files by archiving them to `.codebuddy/sessions/interrupted-{YYYYMMDD}-{HHMMSS}/`, resetting any `in_progress` tasks from the interrupted session back to `pending`. Check for and enforce the concurrency guard via `.lock` file. Create `.codebuddy/sessions/__live_session__/` directory containing:
...
- `execution_pointer.md` at `~/.codebuddy/tasks/{TASK_LIST_ID}/` — created immediately with absolute path to `.codebuddy/sessions/__live_session__/`
  - `{TASK_LIST_ID}` is the identifier of the current task list (visible from `TaskList`). If the runtime does not expose such an identifier, use the task-list name — or a constant like `default` — as the directory name, and keep it consistent for the whole session.

### Step 6: Present Execution Plan and Confirm
Display the plan showing tasks to execute, blocked tasks, and completed count. Also display the details of step 5 which includes the session directory path (`.codebuddy/sessions/__live_session__/`) and files created, including the execution pointer file location.
...
Read `.codebuddy/sessions/__live_session__/execution_context.md` (created in Step 5). If a prior execution context exists, look in `.codebuddy/sessions/` for the most recent timestamped subfolder and merge relevant learnings into the new one.
...
Display execution results with pass/fail counts, total execution time (sum of per-task `duration_ms`), failed task list, newly unblocked tasks, and total token usage (sum of per-task `total_tokens` captured via `TaskOutput`). Save `session_summary.md` to `.codebuddy/sessions/__live_session__/`. Archive the session by moving all contents from `__live_session__/` to `.codebuddy/sessions/{task_execution_id}/`, leaving `__live_session__/` as an empty directory. `execution_pointer.md` stays pointing to `__live_session__/`.

### Step 10: Update CODEBUDDY.md
Review execution context for project-wide changes (new patterns, dependencies, commands, structure changes, design decisions). Make targeted edits to CODEBUDDY.md if meaningful changes occurred. Skip if only task-specific or internal implementation details.
...
- Read `.codebuddy/sessions/__live_session__/execution_context.md` for learnings from prior tasks
...
- Read `CODEBUDDY.md` for project conventions
...
- Write learnings to `.codebuddy/sessions/__live_session__/context-task-{id}.md` (files discovered, patterns learned, issues encountered)
- Write compact result to `.codebuddy/sessions/__live_session__/result-task-{id}.md` (status, verification summary, files modified, issues) — this is the **last** file written and signals completion to the orchestrator
...
Tasks within an execution session share learnings through `.codebuddy/sessions/__live_session__/execution_context.md`:
...
- **Configurable parallelism**: Default 5 concurrent tasks, configurable via `--max-parallel` argument or `.codebuddy/agent-alchemy.local.md` settings. Set to 1 for sequential execution.
