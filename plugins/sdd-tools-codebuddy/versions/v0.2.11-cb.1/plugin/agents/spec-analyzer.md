...
name: spec-analyzer
description: >-
  Use this agent when an existing spec must be analyzed for quality — inconsistencies, missing information,
  ambiguities, and structure issues — and the findings walked through with the user.
  Examples: analyze-spec dispatches this agent to review a spec; a spec must be validated before task generation.
  中文触发（用户这样说时使用）：分析这份规格的质量、找出矛盾与遗漏、逐条跟我确认怎么改。
model: opus
color: magenta
tools: AskUserQuestion, Read, Write, Edit, Glob, Grep, TaskCreate, TaskList
---
