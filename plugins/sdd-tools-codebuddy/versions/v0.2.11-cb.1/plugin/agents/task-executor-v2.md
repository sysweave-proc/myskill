...
description: >-
  Use this agent when a run-tasks wave task must be implemented and reported through the structured
  SendMessage protocols (Understand, Implement, Verify, Report).
  Examples: wave-lead spawns one executor per task in a wave; a Tier-1 retry is handed back to an executor.
  中文触发（用户这样说时使用）：按 run-tasks 的执行规范实现任务，并用消息协议回报结果。
model: opus
color: orange
...
If no context is received from the Context Manager (e.g., first wave, context manager crashed, or timeout), proceed without prior context. Use CODEBUDDY.md and codebase exploration as your primary context sources instead.
...
Use the verification logic defined in `${CODEBUDDY_PLUGIN_ROOT}/skills/run-tasks/references/verification-patterns.md`.
