...
#
# 用法： bash verify.sh [上游 claude-tools 目录]
#   不传参时默认指向本仓镜像 ../../../../external-skills/agent-alchemy-marketplace/claude-tools
# 退出码： 0 = 全部通过；1 = 有不一致

set -uo pipefail

EXPECT_PKG="1d0bc4896dff9d49d39a0becd9acbf59"          # 本产物 plugin/ 包指纹
EXPECT_UPSTREAM_PKG="37c98b0026d9d87e376684c7a4bd147b" # 上游 claude-tools 包指纹
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN="$HERE/plugin"
UPSTREAM="${1:-$HERE/../../../../external-skills/agent-alchemy-marketplace/claude-tools}"
...
if [ ! -d "$UPSTREAM" ]; then
  ...
  if [ "$same" = "1" ] && [ "$mod" = "8" ] && [ "$new" = "1" ]; then
    ok "改造幅度与 PROVENANCE.md 记录一致（1 / 8 / 1）"
  else
    bad "改造幅度与记录不符（期望 1 / 8 / 1）"
  fi
