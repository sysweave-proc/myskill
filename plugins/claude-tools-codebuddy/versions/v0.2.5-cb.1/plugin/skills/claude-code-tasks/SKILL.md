...
description: Reference for CodeBuddy's 6 Task Management tools — TaskCreate, TaskGet, TaskList, TaskUpdate (structured tracking) and TaskOutput, TaskStop (background execution). Covers tool parameters, status lifecycle, completion rules, dependency management, and conventions 中文触发（用户这样说时使用）：任务怎么建、任务状态怎么流转、任务依赖怎么写、后台任务怎么监控、TaskCreate/TaskUpdate/TaskList 的参数。
version: 0.2.5
---

# CodeBuddy Tasks Reference

> **移植说明（本产物新增）**：本文档源自 Claude Code 生态，描述 **Task 管理原语本身** ——
> 工具名、参数、状态生命周期、依赖与完成规则在 CodeBuddy 中同名同样存在，故正文参数表按原样保留。
> 少数**平台注入的环境变量名**（如 `CLAUDE_CODE_*`）按上游原样引用：是否需要改名，请以本机运行时
> 实际提供的环境变量为准（`echo $CODEBUDDY_PLUGIN_ROOT` 之类的探测优先于文档假设）。

This skill is a shared reference for CodeBuddy's 6 Task Management tools. Load it when your skill or agent needs to create, manage, or coordinate tasks.
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-tasks/references/task-patterns.md
```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-tasks/references/anti-patterns.md
```
