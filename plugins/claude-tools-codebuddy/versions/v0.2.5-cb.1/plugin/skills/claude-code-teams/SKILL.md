...
description: Reference for CodeBuddy Agent Teams — lifecycle, messaging, spawning, orchestration, and hooks 中文触发（用户这样说时使用）：怎么建团队、怎么给队友派活、队友之间怎么通信、团队生命周期与 hook 事件。
version: 0.2.5
---

# CodeBuddy Agent Teams Reference

> **移植说明（本产物新增）**：本文档源自 Claude Code 生态，描述 **Agent Teams 原语本身** ——
> `TeamCreate` / `Task(name:, team_name:)` / `SendMessage` / `shutdown_request` 等工具在 CodeBuddy 中
> 同名存在，生命周期与消息协议按原样保留。少数**平台注入的环境变量名**（如 `CLAUDE_CODE_TEAM_NAME`、
> `CLAUDE_TEAMMATE_ID`）按上游原样引用；是否需要改名，请以本机运行时实际提供的环境变量为准。

A shared reference for CodeBuddy's Agent Teams features. Load this skill when building skills or agents that create teams, spawn teammates, coordinate multi-agent workflows, or integrate with team hooks.
...
> **Cross-reference**: For CodeBuddy Tasks (TaskCreate, TaskGet, TaskUpdate, TaskList), see the companion skill: `Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-tasks/SKILL.md`
...
**Cleanup behavior**: Removes the team directory (`~/.codebuddy/teams/{team-name}/`) and the task directory (`~/.codebuddy/tasks/{team-name}/`), clears inbox directories, and deregisters the team.
...
**2. Create tasks**: Team lead uses `TaskCreate` to add tasks. Tasks auto-use the team's task list at `~/.codebuddy/tasks/{team-name}/`.

**3. Spawn teammates**: Team lead uses the `Agent` tool with `team_name` and `name` parameters to spawn teammates. Each teammate runs as an isolated CodeBuddy session. Members can be spawned incrementally as work demands change.
...
Teammates are spawned using the **Agent tool** with the `team_name` parameter. Each teammate runs as a separate CodeBuddy session.

> **Plugin tool name**: In CodeBuddy plugin frontmatter (`allowed-tools` in skills, `tools` in agents), the Agent tool is listed as `Task`. Both names refer to the same spawning capability with identical parameters (`prompt`, `team_name`, `name`, `description`, `subagent_type`, `run_in_background`). Use `Task` in frontmatter definitions and `Agent` or `Task` in skill/agent body instructions.
...
Each teammate runs in its own CodeBuddy session with:
...
- CodeBuddy sends automatic idle notifications to the team lead when a teammate becomes idle
...
CodeBuddy automatically sets these environment variables in every teammate session:
...
CodeBuddy supports multiple backends for running teammate sessions.
...
The backend is configured at the CodeBuddy application level, not per-team or per-spawn call.
...
The team configuration file at `~/.codebuddy/teams/{team-name}/config.json` contains:
...
Messages sent via `SendMessage` are written as JSON files to the recipient's inbox directory. CodeBuddy monitors these directories and delivers messages automatically — teammates do not poll for messages.

### Task Files

Each teammate's assigned work is tracked as task files under `~/.codebuddy/tasks/{team-name}/`. These follow the standard CodeBuddy Tasks format (same as non-team tasks). The task directory is removed when `TeamDelete` is called.
...
**Message delivery is automatic** — CodeBuddy writes message files to recipient inboxes and notifies them. No polling required.
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-teams/references/messaging-protocol.md
```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-teams/references/orchestration-patterns.md
```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/claude-code-teams/references/hooks-integration.md
```
