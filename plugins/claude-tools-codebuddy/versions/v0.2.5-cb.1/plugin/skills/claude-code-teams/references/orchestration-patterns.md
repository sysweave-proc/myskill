...
Proven multi-agent workflow patterns for CodeBuddy Agent Teams. Each pattern defines a team structure, task design, and communication flow for a specific class of problems. Use the [Pattern Selection Guide](#pattern-selection-guide) at the end to choose the right pattern for your use case.
...
Agent(
  prompt="Explore the authentication module (src/auth/). Analyze all files, document public APIs, identify patterns and issues. Write findings to .codebuddy/sessions/analysis/auth-findings.md",
...
Agent(
  prompt="Explore the database module (src/db/). Analyze all files, document schemas, identify patterns and issues. Write findings to .codebuddy/sessions/analysis/db-findings.md",
  team_name="analysis-team",
  name="db-analyst",
  subagent_type="fast",
  run_in_background=true
...
Agent(
  prompt="Explore the API module (src/api/). Analyze all files, document endpoints, identify patterns and issues. Write findings to .codebuddy/sessions/analysis/api-findings.md",
  team_name="analysis-team",
  name="api-analyst",
  subagent_type="fast",
  run_in_background=true
...
Agent(
  prompt="Read analysis findings from .codebuddy/sessions/analysis/*.md. Synthesize into a unified report covering cross-cutting concerns, contradictions, and recommendations. Write to .codebuddy/sessions/analysis/synthesis.md",
  team_name="analysis-team",
  name="synthesizer",
...
TaskCreate(
  ...
  description="Research OAuth2 and JWT patterns for our tech stack. Document findings in .codebuddy/pipeline/research.md",
  status="pending"
)
...
TaskCreate(
  ...
  description="Read .codebuddy/pipeline/research.md. Design the auth architecture. Write design to .codebuddy/pipeline/design.md",
  status="pending",
  blockedBy=["task-1"]
)
...
TaskCreate(
  ...
  description="Read .codebuddy/pipeline/design.md. Implement the auth module following the design. Create files in src/auth/.",
  status="pending",
  blockedBy=["task-2"]
)
...
TaskCreate(
  ...
  description="Review the auth implementation in src/auth/ against the design in .codebuddy/pipeline/design.md. Write review to .codebuddy/pipeline/review.md",
  status="pending",
  blockedBy=["task-3"]
)
...
Agent(
  prompt="Execute task-1: Research authentication patterns. Write findings to .codebuddy/pipeline/research.md",
...
Agent(
  prompt="Execute task-2: Design authentication architecture based on .codebuddy/pipeline/research.md. Write to .codebuddy/pipeline/design.md",
  team_name="feature-pipeline",
  name="architect",
  subagent_type="default"
)
...
Agent(
  prompt="Execute task-3: Implement authentication following .codebuddy/pipeline/design.md",
  team_name="feature-pipeline",
  name="implementer",
  subagent_type="fast"
)
...
Agent(
  prompt="Explore the existing authentication code in src/auth/. Document: file structure, public APIs, middleware chain, session management approach. Write findings to .codebuddy/research/current-auth.md",
...
Agent(
  prompt="Research OAuth2 implementation patterns for Express.js with Passport.js. Document: recommended strategy, required dependencies, configuration patterns. Write findings to .codebuddy/research/oauth2-patterns.md",
  team_name="feature-team",
...
Agent(
  prompt="Analyze integration points that would be affected by adding OAuth2: routes, middleware, session store, environment variables, test setup. Write findings to .codebuddy/research/integration-points.md",
  team_name="feature-team",
...
# Lead reads all .codebuddy/research/*.md files
# Lead produces implementation plan at .codebuddy/research/implementation-plan.md

# Phase 4: Implement (Opus for complex implementation)
Agent(
  prompt="Read the implementation plan at .codebuddy/research/implementation-plan.md and all research files in .codebuddy/research/. Implement OAuth2 authentication following the plan. Create/modify files as specified in the plan.",
  team_name="feature-team",
  name="implementer",
  subagent_type="default"
)
...
- **Human-in-the-loop**: The lead can be a human using CodeBuddy's interactive mode; the planner sends the plan and waits for the human's response
...
Agent(
  prompt="Explore authentication and authorization code. Document: auth flows, token handling, permission checks, session management. Write to .codebuddy/audit/auth-findings.md. Stay available for follow-up questions from the synthesizer.",
...
Agent(
  prompt="Explore input validation and data sanitization. Document: validation patterns, SQL injection protection, XSS prevention, file upload handling. Write to .codebuddy/audit/input-findings.md. Stay available for follow-up questions from the synthesizer.",
  team_name="audit-team",
  name="input-explorer",
  subagent_type="fast",
  run_in_background=true
...
Agent(
  prompt="Explore dependency security and configuration. Document: known vulnerable dependencies, environment variable handling, secrets management, CORS configuration. Write to .codebuddy/audit/config-findings.md. Stay available for follow-up questions from the synthesizer.",
  team_name="audit-team",
...
Agent(
  prompt="You are the security audit synthesizer. Wait for all explorer findings to be written, then: 1) Read all files in .codebuddy/audit/. 2) Cross-reference findings for contradictions or gaps. 3) Send follow-up questions to specific explorers via SendMessage if clarification is needed. 4) Produce the final audit report at .codebuddy/audit/security-report.md with severity ratings and remediation priorities.",
  team_name="audit-team",
  name="synthesizer",
  subagent_type="default",
  run_in_background=true
