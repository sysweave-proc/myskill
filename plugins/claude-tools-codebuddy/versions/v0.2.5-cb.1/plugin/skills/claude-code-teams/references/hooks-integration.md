...
| `session_id` | string | The teammate's CodeBuddy session identifier. Unique per spawn; can be used to correlate logs and artifacts. |
...
TASKS_DIR="$HOME/.codebuddy/tasks/$TASK_LIST_ID"
if [ -d "$TASKS_DIR" ]; then
...
Hooks communicate their result back to CodeBuddy through exit codes. The behavior is consistent across all hook types (command, prompt, agent).
...
Team hook events are configured in the plugin's `hooks.json` file, following the same structure as other CodeBuddy hooks. The `hooks.json` file lives at `{plugin-root}/hooks/hooks.json`.
    ...
    "TeammateIdle": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/hooks/check-idle-quality.sh",
            "timeout": 10
          }
        ]
      }
    ...
    "TaskCompleted": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/hooks/verify-task-output.sh",
            "timeout": 30
          }
        ]
      }
...
| `hooks.<Event>[].hooks[].command` | string | Conditional | Shell command to execute. Required when `type` is `"command"`. Supports `${CODEBUDDY_PLUGIN_ROOT}` for path resolution. |
    ...
    "TaskCompleted": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/hooks/check-output-files.sh",
            "timeout": 10
          },
          {
            "type": "command",
            "command": "bash ${CODEBUDDY_PLUGIN_ROOT}/hooks/run-tests.sh",
            "timeout": 60
          },
          {
            "type": "prompt",
