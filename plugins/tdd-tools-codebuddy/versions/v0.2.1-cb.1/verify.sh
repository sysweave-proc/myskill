...
#
# 用法： bash verify.sh [上游 tdd-tools 目录]
#   不传参时默认指向本仓镜像 ../../../../external-skills/agent-alchemy-marketplace/tdd-tools
# 退出码： 0 = 全部通过；1 = 有不一致

set -uo pipefail

EXPECT_PKG="9a3e892b6b6deb735d87164667592cfa"          # 本产物 plugin/ 包指纹
EXPECT_UPSTREAM_PKG="fcfafcbfd6d2b114663a85675b0648a6" # 上游 tdd-tools 包指纹
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN="$HERE/plugin"
UPSTREAM="${1:-$HERE/../../../../external-skills/agent-alchemy-marketplace/tdd-tools}"
...
if [ ! -d "$UPSTREAM" ]; then
  ...
  if [ "$same" = "4" ] && [ "$mod" = "16" ] && [ "$new" = "6" ]; then
    ok "改造幅度与 PROVENANCE.md 记录一致（4 / 16 / 6）"
  else
    bad "改造幅度与记录不符（期望 4 / 16 / 6）"
  fi
