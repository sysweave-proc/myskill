---
name: tdd-cycle
description: Run a full RED-GREEN-REFACTOR TDD workflow for a feature. Accepts a feature description, task ID, or spec section as input. Presents a plan for confirmation then runs autonomously through all TDD phases. 中文触发（用户这样说时使用）：走一遍红绿重构、TDD 开发这个功能、测试驱动实现、先写测试再实现。
version: 0.2.1
  ...
  questions:
    - header: "TDD Input"
      ...
      options:
        ...
        - label: "Task ID"
          description: "Run TDD for a CodeBuddy Task"
        - label: "Spec section"
...
```
Read: CODEBUDDY.md
Read: .codebuddy/agent-alchemy.local.md (if it exists)
```
...
Read TDD settings from `.codebuddy/agent-alchemy.local.md` if it exists:
...
- Check `.codebuddy/agent-alchemy.local.md` for `tdd.framework`
...
```
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/generate-tests/references/test-patterns.md
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/generate-tests/references/framework-templates.md
```
...
```
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/tdd-cycle/references/tdd-workflow.md
```
...
5. **Follow project conventions**: Read `CODEBUDDY.md` rules, match naming, match file organization
...
The following settings in `.codebuddy/agent-alchemy.local.md` affect the TDD cycle:
