...
1. **Load project conventions**: Read `CODEBUDDY.md`, `.codebuddy/agent-alchemy.local.md` for project-specific rules
2. **Identify test framework**: Auto-detect from config files (`pyproject.toml`, `package.json`, `conftest.py`, `jest.config.*`, `vitest.config.*`). Fall back to `.codebuddy/agent-alchemy.local.md` settings if detection fails
...
The strictness level controls how the RED phase handles tests that pass before implementation. Configure via `.codebuddy/agent-alchemy.local.md` under `tdd.strictness`.
