...
description: Transform SDD tasks into test-first TDD task pairs. Reads existing tasks from /create-tasks and generates paired test tasks with RED-GREEN dependencies. Use when user says "create tdd tasks", "add tdd pairs", "convert to tdd", or wants to apply test-first ordering to SDD tasks. 中文触发（用户这样说时使用）：把任务改成 TDD 任务对、加测试先行任务、转成红绿配对、给已有任务组补测试任务。
version: 0.2.1
...
**CRITICAL**: This skill transforms tasks, NOT creates an implementation plan. When invoked during CodeBuddy's plan mode:
