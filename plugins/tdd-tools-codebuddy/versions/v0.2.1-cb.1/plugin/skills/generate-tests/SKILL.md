---
name: generate-tests
description: Generate test files from acceptance criteria or existing code. Supports criteria-driven mode (from specs/tasks) and code-analysis mode (from source files). Auto-detects test framework and spawns test-writer agents for parallel generation. 中文触发（用户这样说时使用）：生成测试文件、按验收标准写测试、给这个模块补单测、生成 pytest/Jest/Vitest 测试。
version: 0.2.1
...
If detection is still ambiguous, check `.codebuddy/agent-alchemy.local.md` for:
```yaml
tdd:
  ...
  questions:
    - header: "Test Generation Input"
      ...
      options:
        ...
        - label: "Task ID"
          description: "Generate tests from a CodeBuddy Task's acceptance criteria"
...
The following settings in `.codebuddy/agent-alchemy.local.md` affect test generation:
