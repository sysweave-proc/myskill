...
3. Settings fallback (low confidence)
   └── .codebuddy/agent-alchemy.local.md --> framework setting
...
When auto-detection fails or is ambiguous, check `.codebuddy/agent-alchemy.local.md`:
...
| 14 | `.codebuddy/agent-alchemy.local.md` setting | Per config | Low |
...
2. **Generate tests with a framework comment header:**
   ...
   # If this is incorrect, configure your framework in .codebuddy/agent-alchemy.local.md
   ```
