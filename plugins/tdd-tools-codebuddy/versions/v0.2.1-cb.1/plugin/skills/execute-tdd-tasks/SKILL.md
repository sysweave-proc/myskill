...
**CRITICAL: Complete ALL 9 steps.** The workflow is not complete until Step 9: Update CODEBUDDY.md is evaluated. After completing each step, immediately proceed to the next step without waiting for user prompts (except Step 4 which requires user confirmation).
...
```
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/execute-tdd-tasks/references/tdd-execution-workflow.md
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/execute-tdd-tasks/references/tdd-verification-patterns.md
```
...
2. `max_parallel` in `.codebuddy/agent-alchemy.local.md`
3. Default: 5
...
Read `.codebuddy/agent-alchemy.local.md` if it exists, for TDD-specific settings:
...
1. Check if `.codebuddy/sessions/__live_session__/` contains leftover files
2. If found, archive to `.codebuddy/sessions/interrupted-{YYYYMMDD}-{HHMMSS}/`
3. Reset any `in_progress` tasks from the interrupted session to `pending`

**Concurrency guard:** Check for `.codebuddy/sessions/__live_session__/.lock`. Follow the same lock protocol as `execute-tasks`.

**Create session files in `.codebuddy/sessions/__live_session__/`:**
...
5. **`progress.md`** -- Initialize with status template:
   ...
   ```
6. **`execution_pointer.md`** at `$HOME/.codebuddy/tasks/{TASK_LIST_ID}/execution_pointer.md` -- Absolute path to `.codebuddy/sessions/__live_session__/`
...
Read `.codebuddy/sessions/__live_session__/execution_context.md` (created in Step 6).
...
Read `.codebuddy/sessions/__live_session__/execution_context.md` and hold as baseline for this wave. All agents read from the same snapshot.
...
5. **Poll for completion**: After launching all background agents, poll for result files using `poll-for-results.sh` from `execute-tasks`. The script checks for `result-task-{id}.md` files every 15 seconds for up to 45 minutes, printing progress lines periodically. A single Bash invocation handles the entire polling lifecycle.
   ...
   **Poll invocation** (via Bash tool with `timeout: 2760000`):
   ```bash
   bash ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-sdd-tools/skills/execute-tasks/scripts/poll-for-results.sh \
     .codebuddy/sessions/__live_session__ {task_ids...}
   ```
...
2. **Read result files**: For each task in the wave, read `.codebuddy/sessions/__live_session__/result-task-{id}.md`. Parse status, attempt, verification, files modified, and issues. For TDD tasks, also parse the `## TDD Compliance` section (RED Verified, GREEN Verified, Refactored, Coverage Delta).
...
1. Archive completed task files to `.codebuddy/sessions/__live_session__/tasks/`
...
1. Save `session_summary.md` to `.codebuddy/sessions/__live_session__/` with full summary content
2. Archive the session: move all contents from `__live_session__/` to `.codebuddy/sessions/{task_execution_id}/`
...
Review `.codebuddy/sessions/{task_execution_id}/execution_context.md` for project-wide changes.
...
**Strictness levels** (from `.codebuddy/agent-alchemy.local.md` `tdd.strictness` setting):
