...
description: Analyze test coverage and identify gaps with actionable recommendations 中文触发（用户这样说时使用）：看测试覆盖率、覆盖率缺口在哪、哪些分支没测到、给我补测建议。
version: 0.2.1
...
```
Read: ${CODEBUDDY_PLUGIN_ROOT}/skills/analyze-coverage/references/coverage-patterns.md
```
...
- **Threshold source (in precedence order)**:
  ...
  2. `.codebuddy/agent-alchemy.local.md` `tdd.coverage-threshold`
  3. Default: 80%
...
The following settings in `.codebuddy/agent-alchemy.local.md` affect coverage analysis:
