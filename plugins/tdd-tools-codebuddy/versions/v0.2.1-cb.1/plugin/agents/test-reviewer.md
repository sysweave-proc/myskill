...
name: test-reviewer
description: >-
  Use this agent when AI-generated test files must be evaluated against the behavior-driven test rubric,
  with confidence-scored findings and dimension-based quality scoring.
  Examples: tdd-cycle wants the generated tests judged before moving on; a test file looks assertion-heavy
  but behavior-blind and needs an independent audit.
  中文触发（用户这样说时使用）：评审这些测试用例、按测试评分表打分、看看测试是不是只覆盖实现细节、给测试质量意见。
model: opus
color: red
