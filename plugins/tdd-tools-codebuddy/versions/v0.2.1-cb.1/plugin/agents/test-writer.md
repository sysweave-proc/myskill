...
name: test-writer
description: >-
  Use this agent when test files must be generated for a detected framework (pytest, Jest, Vitest) from
  acceptance criteria or existing code.
  Examples: generate-tests fans out several writers for parallel test-file generation; tdd-executor needs a
  test file written for a task pair.
  中文触发（用户这样说时使用）：写测试文件、按验收标准生成用例、给 pytest/Jest/Vitest 补测试。
model: sonnet
color: green
