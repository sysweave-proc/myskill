...
#   - $HOME/.codebuddy/tasks/*/execution_pointer.md
#   - */.codebuddy/sessions/* (session files within any project)
#
# Approves Bash commands targeting .codebuddy/sessions/
#
...
case "$tool_name" in
  Write|Edit)
    ...
    # Match execution_pointer.md in ~/.codebuddy/tasks/*/
    if [[ "$file_path" == "$HOME/.codebuddy/tasks/"*/execution_pointer.md ]]; then
      approve "execution_pointer.md"
    fi

    # Match any file inside .codebuddy/sessions/ (absolute or relative paths)
    if [[ "$file_path" == */.codebuddy/sessions/* ]] || [[ "$file_path" == .codebuddy/sessions/* ]]; then
  ...
  Bash)
    ...
    # Match any bash command targeting .codebuddy/sessions/
    if [[ "$command" == *".codebuddy/sessions/"* ]]; then
