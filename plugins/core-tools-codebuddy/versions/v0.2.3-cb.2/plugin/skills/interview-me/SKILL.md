---
name: interview-me
description: Conduct an adaptive interactive interview on any topic and produce a structured markdown report, summary, or implementation plan. Use when the user says "interview me", "interview me about", "run an interview on", "help me think through", or wants guided discovery on any subject. Also triggers on "talk me through", "explore X with me", "help me plan Y", or "walk me through my thinking on Z" when the user wants a guided conversation rather than a direct answer. Applies broadly — career decisions, project ideas, product direction, personal goals, research questions, team problems — not just technical topics. 中文触发（用户这样说时使用）：访谈我、采访我、帮我把思路理清、带我把这件事想透、引导式提问帮我做决定。
version: 0.2.3
allowed-tools: AskUserQuestion, Task, Read, Write, Glob, Grep, Bash
---

> **Reference loading.** `${CODEBUDDY_PLUGIN_ROOT}` is the CodeBuddy plugin-root variable. If your
> runtime does not expand it, locate the same file by globbing `**/agent-alchemy-core-tools/skills/**`,
> or read the equivalent path relative to this skill's own directory (`references/<file>.md`).
...
This skill generates a markdown artifact *as its own planning activity*. When invoked during CodeBuddy's Plan mode:
...
- `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/question-bank.md` — seed questions organized by category and depth
- `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/research-triggers.md` — when to fire the researcher

The three output templates live at `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/templates/`. Load the one matching the user's chosen output type in Phase 5.
...
Read `.codebuddy/agent-alchemy.local.md` if it exists. Extract the `interview-me` section. Apply these defaults when a value is missing:
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/question-bank.md
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/research-triggers.md
```
...
|---|---|
| `report-detailed` | `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/templates/report-detailed.md` |
| `report-summary` | `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/templates/report-summary.md` |
| `implementation-plan` | `${CODEBUDDY_PLUGIN_ROOT}/skills/interview-me/references/templates/implementation-plan.md` |
