...
#   - */.codebuddy/sessions/__da_live__/* (active session files)
#   - */.codebuddy/sessions/exploration-cache/* (exploration cache)
#   - */.codebuddy/sessions/da-*/* (archived sessions)
#
...
#
# IMPORTANT: This hook must NEVER exit non-zero. A non-zero exit causes CodeBuddy
...
is_da_session_path() {
  local path="$1"
  if [[ "$path" == */.codebuddy/sessions/__da_live__/* ]] || [[ "$path" == .codebuddy/sessions/__da_live__/* ]]; then
    return 0
  fi
  if [[ "$path" == */.codebuddy/sessions/exploration-cache/* ]] || [[ "$path" == .codebuddy/sessions/exploration-cache/* ]]; then
    return 0
  fi
  if [[ "$path" == */.codebuddy/sessions/da-*/* ]] || [[ "$path" == .codebuddy/sessions/da-*/* ]]; then
    return 0
  fi
  return 1
}
...
case "$tool_name" in
  ...
  Bash)
    ...
    if [[ "$command" == *".codebuddy/sessions/__da_live__"* ]] || \
       [[ "$command" == *".codebuddy/sessions/exploration-cache"* ]] || \
       [[ "$command" == *".codebuddy/sessions/da-"* ]]; then
