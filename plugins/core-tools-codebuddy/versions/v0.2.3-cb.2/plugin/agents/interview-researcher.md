...
description: |
  ...
  中文触发（用户这样说时使用）：帮我查一下这个方向的资料、有哪些现成做法、合规与最佳实践怎么说。
model: opus
color: blue
tools: WebSearch, WebFetch
---

> **Optional Context7 MCP.** If a Context7 MCP server is configured in this environment
> (tool names look like `mcp__*context7*__resolve-library-id` and `mcp__*context7*__query-docs`),
> prefer it for library/framework documentation, then fall back to WebFetch and WebSearch.
> If Context7 is unavailable, skip it silently — never block on it.
