...
#
# 用法： bash verify.sh [上游 dev-tools 目录]
#   不传参时默认指向本仓镜像 ../../../../external-skills/agent-alchemy-marketplace/dev-tools
# 退出码： 0 = 全部通过；1 = 有不一致

set -uo pipefail

EXPECT_PKG="b2fd971573a6b02078c6bb5e3edb5323"          # 本产物 plugin/ 包指纹
EXPECT_UPSTREAM_PKG="a9c9403919c7a3154afc000922a2f00b" # 上游 dev-tools 包指纹
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN="$HERE/plugin"
UPSTREAM="${1:-$HERE/../../../../external-skills/agent-alchemy-marketplace/dev-tools}"
...
if [ ! -d "$UPSTREAM" ]; then
  ...
  if [ "$same" = "9" ] && [ "$mod" = "14" ] && [ "$new" = "6" ]; then
    ok "改造幅度与 PROVENANCE.md 记录一致（9 / 14 / 6）"
  else
    bad "改造幅度与记录不符（期望 9 / 14 / 6）"
  fi
  ...
  if [ "$missing" = "hooks/hooks.json hooks/resolve-cross-plugins.sh" ]; then
    ok "未移植项与记录一致（hooks/hooks.json、hooks/resolve-cross-plugins.sh，属有意不移植）"
  else
