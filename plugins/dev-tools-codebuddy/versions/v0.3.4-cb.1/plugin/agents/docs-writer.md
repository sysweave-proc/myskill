...
name: docs-writer
description: >-
  Use this agent when documentation must be written or updated from codebase findings — API references,
  architecture guides, standalone markdown files, or change summaries (MkDocs-flavored or GitHub-flavored).
  Examples: docs-manager needs files generated for a MkDocs site; an architecture guide must be rewritten
  to match the current design.
  中文触发（用户这样说时使用）：写 API 文档、写架构说明、补 README、生成 MkDocs 或 GitHub 风格 markdown 文档、写变更摘要。
model: opus
color: blue
tools: Read, Glob, Grep, Bash
---

> **Required knowledge loading.** Before writing docs or diagrams, load the shared knowledge skill:
> ```
> Read ${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-core-tools/skills/technical-diagrams/SKILL.md
> ```
> If that path does not resolve, glob `**/agent-alchemy-core-tools/skills/technical-diagrams/SKILL.md`
