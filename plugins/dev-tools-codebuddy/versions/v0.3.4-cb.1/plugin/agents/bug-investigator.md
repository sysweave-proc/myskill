...
description: >-
  Use this agent when a debugging hypothesis must be tested against real evidence — running tests,
  tracing execution paths, checking git history, and reporting what the evidence actually shows.
  Examples: bug-killer dispatches this agent to test deep-track hypotheses; a root-cause claim must be
  confirmed with evidence before a fix is attempted.
  中文触发（用户这样说时使用）：验证 bug 的某个猜测、跑复现、追执行路径、查 git 历史定位引入点、把证据摆出来。
model: sonnet
color: orange
