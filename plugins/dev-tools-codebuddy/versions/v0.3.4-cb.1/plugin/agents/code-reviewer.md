...
name: code-reviewer
description: >-
  Use this agent when an implementation must be reviewed for correctness, security, and maintainability
  with confidence-scored findings.
  Examples: feature-dev spawns several reviewers in parallel for independent perspectives; a risky change
  needs a second opinion before merge.
  中文触发（用户这样说时使用）：评审这段实现、帮我挑问题、检查正确性与安全性、给发现打分、换个视角再审一遍。
model: opus
color: red
