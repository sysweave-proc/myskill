...
description: >-
  Use this agent when CHANGELOG.md must be reviewed and updated from git history — grouping commits
  into Keep a Changelog sections for the [Unreleased] entry.
  Examples: feature-dev finished a change and needs a changelog entry; a release needs the Unreleased
  section finalized before tagging.
  中文触发（用户这样说时使用）：根据提交记录更新 CHANGELOG、整理 [Unreleased]、按 Keep a Changelog 归类变更。
model: sonnet
color: green
