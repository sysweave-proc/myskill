---
name: bug-killer
description: Systematic, hypothesis-driven debugging workflow with triage-based track routing. Use when asked to "fix this bug", "debug this", "why is this failing", "this is broken", "investigate this error", "track down this issue", or any debugging situation. Supports --deep flag to force full investigation. 中文触发（用户这样说时使用）：修 bug、排查报错、这段代码为什么跑不通、帮我调试、定位问题、这个功能坏了、追一下这个异常。（加 --deep 强制完整排查）
version: 0.3.4
...
|----------|---------------|
| Python | `Read ${CODEBUDDY_PLUGIN_ROOT}/skills/bug-killer/references/python-debugging.md` |
| TypeScript / JavaScript | `Read ${CODEBUDDY_PLUGIN_ROOT}/skills/bug-killer/references/typescript-debugging.md` |
| Other / Multiple | `Read ${CODEBUDDY_PLUGIN_ROOT}/skills/bug-killer/references/general-debugging.md` |
...
1. **Load code-quality skill:**
   ```
   Read ${CODEBUDDY_PLUGIN_ROOT}/skills/code-quality/SKILL.md
   ```
...
```
Read ${CODEBUDDY_PLUGIN_ROOT}/skills/project-learnings/SKILL.md
```
