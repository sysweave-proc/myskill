---
name: feature-dev
description: Feature development workflow with exploration, architecture, implementation, and review phases. Use for implementing new features or significant changes. 中文触发（用户这样说时使用）：开发新功能、实现一个特性、加个功能、走一遍完整功能开发流程（探索→提问→架构→实现→评审）。
version: 0.3.4
...
1. **Run deep-analysis workflow:**
   - Read `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-core-tools/skills/deep-analysis/SKILL.md` and follow its workflow
...
1. **Load skills for this phase:**
   - Read `${CODEBUDDY_PLUGIN_ROOT}/skills/architecture-patterns/SKILL.md` and apply its guidance
   - Read `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-core-tools/skills/language-patterns/SKILL.md` and apply its guidance
   - Read `${CODEBUDDY_PLUGIN_ROOT}/../agent-alchemy-core-tools/skills/technical-diagrams/SKILL.md` and apply its styling rules for any Mermaid diagrams in architecture proposals
...
5. **Generate ADR artifact:**
   - Read the ADR template from `${CODEBUDDY_PLUGIN_ROOT}/skills/feature-dev/references/adr-template.md`
...
1. **Load skills for this phase:**
   - Read `${CODEBUDDY_PLUGIN_ROOT}/skills/code-quality/SKILL.md` and apply its guidance
...
2. **Update CHANGELOG.md:**
   - Read the entry template from `${CODEBUDDY_PLUGIN_ROOT}/skills/feature-dev/references/changelog-entry-template.md`
...
- Use `subagent_type: "code-architect"` (cross-plugin, from core-tools)
