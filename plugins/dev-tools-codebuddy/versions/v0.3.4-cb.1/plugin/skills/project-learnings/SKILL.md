...
description: Captures project-specific patterns and anti-patterns into the project's CODEBUDDY.md. Loaded by other skills (bug-killer, feature-dev, etc.) when they discover project-specific knowledge worth encoding for future sessions. 中文触发（用户这样说时使用）：沉淀项目约定、把这次踩的坑写进项目规则、记录本项目特有的模式与反模式。
version: 0.3.4
...
Capture project-specific patterns and anti-patterns into the project's CODEBUDDY.md. This creates a self-improving feedback loop where discoveries from debugging, development, and review make future CodeBuddy sessions smarter.

**CRITICAL:** Only project-specific knowledge qualifies. Generic programming advice does not belong in CODEBUDDY.md.
...
| Is it something CodeBuddy's training data wouldn't cover? | "Never call `db.query()` without the `timeout` option — the default is infinite" | "Use async/await instead of callbacks" |

**If NO to all criteria → STOP.** Do not add generic programming knowledge to CODEBUDDY.md. Return to the calling skill and report that no project-specific learning was found.

**If YES to any → proceed to Step 2.**

---

## Step 2: Read Existing CODEBUDDY.md

1. **Find the project's CODEBUDDY.md:**
   - Check the repository root first
   - If not found, check if there's a project-level `.codebuddy/` directory
...
- Follow the existing CODEBUDDY.md style and conventions
...
2. Where it will be placed in CODEBUDDY.md (section name, after which line/entry)
...
1. Use the Edit tool to add the entry to CODEBUDDY.md at the identified location
