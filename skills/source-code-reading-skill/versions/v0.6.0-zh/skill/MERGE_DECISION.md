...
因此 v0.6.0 不是在再表达件的 12 个文件基础上重新搭包，而是从完整的 v0.5.1 资产集增量演进。

### 决策 B：以再表达件的 Cognitive Core 重构 SKILL.md

v0.5.1 的思想是完整的，但核心入口仍然偏“工程手册”；再表达件对 Agent 的执行心智模型更清晰。
...
再表达件把部分历史材料压缩成原则，这对 Agent 入口有利，但如果直接替换，会丢掉：
...
- `references/situated-knowledge.md` 单独成为情境知识规范，因为这是再表达件真正新增的一块能力。

### 决策 E：图法目录不压缩

再表达件对历史图法采取了收敛，但 v0.5.1 中已经有完整 `references/diagrams/diagram-catalog.md`。为了“不丢东西”的目标，本次不删除它。
...
v0.5.1 已有 evolution 资产，再表达件又进一步把它明确成：
...
### 3.3再表达件有新增能力、v0.5.1 没有

直接增量加入，不回写成含糊的大段原则。

典型：Situated Knowledge。

### 3.4 v0.5.1 有历史资产、再表达件没有
