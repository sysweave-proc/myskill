...
**读后重写版，不属于 [`versions/`](../versions/) 谱系** —— 那六版是同一场会话在六个时刻的冻结，本版是读完它们的产物后自己重写的，血缘不同，故与 `versions/`、`docs/` 平铺并列供对照。

它同时是 v0.6.0 的一手输入：被定位为 `cognitive_execution_source`（认知与执行协议的强化版），与作为工程母版的 v0.5.1 增量合并成 [`versions/v0.6.0`](../versions/v0.6.0/)。
