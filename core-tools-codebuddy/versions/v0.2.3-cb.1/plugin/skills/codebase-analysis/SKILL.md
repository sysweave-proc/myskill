...
2. **Check for cached results:**
   - Check if `.codebuddy/sessions/exploration-cache/manifest.md` exists
   ...
   - **If cache is valid**, use `AskUserQuestion`:
     - **Use cached results** (show the formatted cache date) — Read cached synthesis from `.codebuddy/sessions/exploration-cache/synthesis.md` and recon from `recon_summary.md`. Set `CACHE_HIT = true` and `CACHE_TIMESTAMP` to the cache's timestamp. Skip step 3 and proceed directly to step 4.
...
3. **Run deep-analysis workflow:**
   - Read `${CODEBUDDY_PLUGIN_ROOT}/skills/deep-analysis/SKILL.md` and follow its workflow
...
2. **Load report template:**
   - Read `${CODEBUDDY_PLUGIN_ROOT}/skills/codebase-analysis/references/report-template.md`
...
- **Update project documentation** — Add/update README.md, CODEBUDDY.md, or AGENTS.md with analysis insights
...
- **CODEBUDDY.md** — Add patterns, conventions, critical files, and architectural decisions
...
- **CODEBUDDY.md**: Read existing file at project root. If none exists, ask if one should be created (if declined, skip). Draft updates focusing on patterns, conventions, critical files, and architectural decisions.
...
- Load reference template from `${CODEBUDDY_PLUGIN_ROOT}/skills/codebase-analysis/references/actionable-insights-template.md`
