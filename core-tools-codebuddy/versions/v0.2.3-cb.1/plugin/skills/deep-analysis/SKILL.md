---
name: deep-analysis
description: Deep exploration and synthesis workflow using Agent Teams with dynamic planning and hub-and-spoke coordination. Use when asked for "deep analysis", "deep understanding", "analyze codebase", "explore and analyze", or "investigate codebase". 中文触发（用户这样说时使用）：深度分析、深读代码库、全面分析这个项目、探索并分析、摸清这个代码库。
version: 0.2.3
...
This skill can be invoked standalone or loaded by other skills as a reusable building block. Approval behavior is configurable via `.codebuddy/agent-alchemy.local.md`.
...
1. **Read settings file:**
   - Check if `.codebuddy/agent-alchemy.local.md` exists
...
1. Check if `.codebuddy/sessions/exploration-cache/manifest.md` exists
...
1. Check if `.codebuddy/sessions/__da_live__/checkpoint.md` exists
...
3. Use `AskUserQuestion` to offer:
   ...
   - **"Start fresh"** — Archive the interrupted session to `.codebuddy/sessions/da-interrupted-{timestamp}/` and proceed normally
...
1. Create `.codebuddy/sessions/__da_live__/` directory
...
2. **Rapid codebase reconnaissance:**
   ...
   - **Key documentation:** Read `README.md`, `CODEBUDDY.md`, or similar docs if they exist for project context
...
5. **Checkpoint** (if `ENABLE_CHECKPOINTING = true`):
   - Update `.codebuddy/sessions/__da_live__/checkpoint.md`: set `current_phase: 1`
   - Write `.codebuddy/sessions/__da_live__/team_plan.md` with the full team plan from Step 4
   - Write `.codebuddy/sessions/__da_live__/recon_summary.md` with reconnaissance findings from Step 2
...
4. **Checkpoint** (if `ENABLE_CHECKPOINTING = true`):
   - Update `.codebuddy/sessions/__da_live__/checkpoint.md`: set `current_phase: 2`, record `approval_mode` (approved/auto-approved)
...
5. **Checkpoint** (if `ENABLE_CHECKPOINTING = true`):
   - Update `.codebuddy/sessions/__da_live__/checkpoint.md`: set `current_phase: 3`, record `team_name`, `explorer_names` (list), `task_ids` (map of explorer → task ID), `synthesis_task_id`
...
2. **If task is `completed`**: Record the explorer's findings. If `ENABLE_CHECKPOINTING = true`, write `explorer-{N}-findings.md` to `.codebuddy/sessions/__da_live__/` and update checkpoint.
...
4. **Checkpoint** (if `ENABLE_CHECKPOINTING = true`):
   - Update `.codebuddy/sessions/__da_live__/checkpoint.md`: set `current_phase: 5`
   - Write `.codebuddy/sessions/__da_live__/synthesis.md` with the synthesis results
...
2. **Write exploration cache** (if `CACHE_TTL > 0`):
   - Create `.codebuddy/sessions/exploration-cache/` directory (overwrite if exists)
   - Write `manifest.md`:
     ```markdown
...
5. **Archive session and cleanup team:**
   - If `ENABLE_CHECKPOINTING = true`: Move `.codebuddy/sessions/__da_live__/` to `.codebuddy/sessions/da-{timestamp}/`
...
- If `.codebuddy/agent-alchemy.local.md` exists but is malformed or the `deep-analysis` section is unparseable: warn the user ("Settings file found but could not parse deep-analysis settings — using defaults") and proceed with default approval values.
